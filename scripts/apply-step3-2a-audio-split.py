...
def main():
    ...
    if AUDIO.exists():
        fail("js/core/audio.js already exists; refusing to overwrite")
    if AUDIO_SCRIPT in index_html:
        fail("audio script tag already exists; refusing to reapply")

    start = main_js.find(SOUND_START)
    end = main_js.find(CONFIG_START)
    if start < 0:
        fail("SOUND block start marker not found")
    if end < 0:
    ...
    missing = [m for m in required if sound_block.count(m) != 1]
    if missing:
        fail("unexpected audio marker counts in extracted block: " + ", ".join(missing))
    ...
    patched_index = index_html.replace(MAIN_SCRIPT, AUDIO_SCRIPT + "\n" + MAIN_SCRIPT, 1)
